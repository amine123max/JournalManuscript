...
下面列出的 release 资产，是当前已经完成校验的最小 family。每个 ZIP 都已经对齐到“安装后可直接基于包内 family 模板基线创建 `paper/` 脚手架”。
...
对于暂时没有统一 family 根模板包的出版社，脚手架会优先使用当前仓库里已缓存的最近似官方期刊模板，作为 family 级起稿基线。
