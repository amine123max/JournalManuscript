# Custom Journal Family Profile

Use this profile when the target manuscript follows the Custom Journal Family template family.

## Typical Template Anchor

## Official Guide Display

- Official guide page: Custom journal placeholder
- Downloaded guide file: `assets/official-templates/guides/custom-journal/official-guide-source.txt`
- Guide note: no official guide exists for the custom-journal placeholder
- Guide alignment: use this local guide file as the nearest official reference before calling the layout submission-ready

- Common template anchor: A local class file or submission package not covered elsewhere
- Common traits: Use this as a staging profile when a journal does not yet have its own dedicated folder

## Check Before Editing

1. Exact class file and option set
2. Front matter structure
3. Abstract and keyword requirements
4. Figure and table placement rules
5. Citation and bibliography mode
6. Required end sections and declarations
7. Compile toolchain

## Safe Rule

Treat this file as a family baseline and verify the concrete journal template before making high-risk layout changes.

